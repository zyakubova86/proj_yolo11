...
class BaseSolution:
    ...
    def extract_tracks(self, im0):
        ...
        """
        self.tracks = self.model.track(source=im0, persist=True, classes=self.CFG["classes"], verbose=False, conf=0.9, device=0)
        ...
        if self.track_data and self.track_data.id is not None:
            ...
            self.track_confidences = self.track_data.conf.cpu().tolist()

        else:
            # LOGGER.warning("WARNING ⚠️ no tracks found!")
            self.boxes, self.clss, self.track_ids, self.track_confidences = [], [], [], []
